...
contract MyUSDEngine is Ownable {
    ...
    function repayUpTo(uint256 amount) public {
        ...
        if (shares > s_userDebtShares[msg.sender]) {
            ...
            newAmount = getCurrentDebtValue(msg.sender);
        }

        uint256 balance = i_myUSD.balanceOf(msg.sender);
        if (balance < newAmount || newAmount == 0) {
        ...
        i_myUSD.burnFrom(msg.sender, newAmount);
        emit DebtSharesBurned(msg.sender, newAmount, shares);
    }
